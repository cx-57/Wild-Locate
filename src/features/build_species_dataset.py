...
from src.features.extract import FEATURE_ORDER, extract_features
...
def build_species_dataset(
    ...
    # Fail the build if the global failure rate is high or if either class is materially impacted.
    ...
    print(f"Successful rows: {len(output_df)}")
    print(f"Failed rows: {len(failed_rows)}")
    print(f"Failed presences: {failed_presence_rows}")
    print(f"Failed backgrounds: {failed_background_rows}")
...
def main() -> None:
    ...
    parser.add_argument(
        "--species",
        required=True,
        help="Species name, e.g. 'Fisher' or 'Bobcat'.",
    )
    parser.add_argument(
        "--training-points",
        default=None,
        help="Optional path to the species training-points CSV. Defaults to data/processed/samples/<species>_training_points.csv.",
    )
    parser.add_argument(
        "--output",
        default=None,
        help="Optional output CSV path. Defaults to data/processed/features/species/<species>_features.csv.",
    )
