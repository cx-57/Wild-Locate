...
def main() -> None:
    ...
    parser.add_argument(
        "--species",
        required=True,
        help="Species name used to locate the processed feature dataset.",
    )
    parser.add_argument(
        "--dataset",
        default=None,
        help="Optional path to a species feature CSV. Defaults to data/processed/features/species/<species>_features.csv.",
    )
    ...
    if not dataset_file.exists():
        raise FileNotFoundError(
            f"Could not find species feature dataset at {dataset_file}."
        )
    ...
    for fold_index, (_, val_idx) in enumerate(splits, start=1):
        y_val = y.iloc[val_idx]
        print(
            f"Fold {fold_index}: presence={int(y_val.sum())}, background={int((1 - y_val).sum())}"
        )
    ...
    selected_features = top_features_for_model(
        best_result["model"],
        selected_pipeline,
        feature_columns,
    )
